...
export const Route = createFileRoute("/blog/category/$category")({
  ...
  head: ({ loaderData }) => {
    ...
    const title = m ? `${m.label} Insights | Yetki Engineering Blog` : "Category | Yetki Engineering";
    ...
    return {
      ...
      links: loaderData ? [{ rel: "canonical", href: `/blog/category/${loaderData.cat}` }] : [],
    };
  },
...
function CategoryPage() {
  const { cat, articles, meta } = Route.useLoaderData() as { cat: Category; articles: Article[]; meta: { label: string; desc: string } };
  return (
