...
const services = [
  {
    ...
    text: "Blue light, laser and metrology-grade scanning for industrial components.",
  },
  {
    to: "/reverse-engineering-services",
    title: "Reverse Engineering",
    text: "Convert physical parts into accurate CAD, drawings and manufacturable data.",
  },
  {
    to: "/cad-modelling-services",
    title: "CAD Modelling",
    text: "Parametric modelling, surfacing and product development for production teams.",
  },
  {
    to: "/mould-design-manufacturing",
    title: "Mould Design & Manufacturing",
    text: "Injection mould design, tooling support and mould manufacturing workflows.",
  },
  {
    to: "/precision-manufacturing",
    title: "Precision Manufacturing",
    text: "Prototype and batch manufacturing for industrial components and assemblies.",
  },
  {
    to: "/3d-scanner-sales",
    title: "3D Scanner Sales",
    text: "Industrial 3D scanner supply, onboarding and technical support.",
  },
];
...
function ServicesPage() {
  const hyderabadPages = seoLandingPages.filter((page) => page.path.includes("hyderabad"));
  const indiaPages = seoLandingPages.filter((page) => page.path.includes("india"));
    ...
    >
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {services.map((service) => (
            <Link
              key={service.to}
              to={service.to}
              className="group rounded-2xl glass-strong p-6 hover:bg-blue-500/5"
            >
      ...
      <ContentBand>
        <div className="grid gap-8 lg:grid-cols-2">
          <div>
            <h2 className="mb-5 text-2xl font-semibold">Hyderabad service pages</h2>
            <div className="space-y-3">
              {hyderabadPages.map((page) => (
                <Link
                  key={page.path}
                  to={page.path}
          ...
          <div>
            <h2 className="mb-5 text-2xl font-semibold">India service pages</h2>
            <div className="space-y-3">
              {indiaPages.map((page) => (
                <Link
                  key={page.path}
