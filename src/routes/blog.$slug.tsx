...
import { absoluteUrl, canonicalUrl, seoMeta } from "@/lib/seo";
  ...
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        ...
        ...(a ? [{ property: "article:published_time", content: a.date }] : []),
      ],
      links: a ? [{ rel: "canonical", href: canonicalUrl(`/blog/${a.slug}`) }] : [],
      scripts: a
        ? [
            {
              ...
              children: JSON.stringify({
                ...
                "@type": "BlogPosting",
                mainEntityOfPage: canonicalUrl(`/blog/${a.slug}`),
                headline: a.title,
...
function ArticlePage() {
  ...
  return (
    <article className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-12">
      ...
      <h1 className="mt-3 text-3xl sm:text-4xl lg:text-5xl font-semibold leading-[1.1]">
        {article.title}
      </h1>
      ...
      <div className="mt-8 flex flex-wrap gap-2">
        {article.tags.map((t) => (
          <span key={t} className="rounded-full glass px-3 py-1 text-[11px] text-muted-foreground">
            #{t}
          </span>
        ))}
      </div>
          ...
          <div className="mt-5 grid sm:grid-cols-3 gap-4">
            {related.map((a) => (
              <Link
                ...
                to="/blog/$slug"
                params={{ slug: a.slug }}
                className="group rounded-xl glass p-4 hover:bg-blue-500/5"
              >
                <div className="text-sm font-medium leading-snug">{a.title}</div>
                <div className="mt-3 inline-flex items-center gap-1 text-xs text-blue-300">
                  Read{" "}
                  <ArrowRight
                    size={12}
      ...
      <section className="mt-16 rounded-3xl glass-strong p-8 text-center glow-blue">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Talk to our team about 3D scanning, reverse engineering or precision manufacturing.
        </p>
        <Link
          to="/"
          hash="contact"
          className="mt-5 inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-500 to-blue-700 px-6 py-3 text-sm font-semibold text-white"
        >
