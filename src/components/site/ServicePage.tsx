    ...
    <div className="min-h-screen relative">
      ...
      <section className="relative pt-32 pb-16 overflow-hidden">
        <div className="absolute inset-0 -z-10">
          <img src={p.heroImage} alt="" className="h-full w-full object-cover opacity-40" />
        ...
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="mt-5 max-w-3xl text-base sm:text-lg text-foreground/70 leading-relaxed">{p.intro}</p>
          <div className="mt-7 flex gap-3 flex-wrap">
            <a href="#contact" className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-500 to-blue-700 px-6 py-3 text-sm font-semibold text-white shadow-[0_0_30px_rgba(60,130,255,0.4)]">
              Request a Quote <ArrowRight size={16} />
            </a>
            <Link to="/" className="inline-flex items-center gap-2 rounded-xl glass-strong px-6 py-3 text-sm font-semibold">Back to overview</Link>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 space-y-14">
          {p.sections.map((s, i) => (
            <article key={i} className="grid lg:grid-cols-12 gap-8 items-start">
              <div className="lg:col-span-4">
                <div className="text-[11px] uppercase tracking-[0.22em] text-blue-300">Section {String(i + 1).padStart(2, "0")}</div>
              ...
              <div className="lg:col-span-8 glass-strong rounded-2xl p-6">
                <p className="text-sm sm:text-base text-foreground/80 leading-relaxed whitespace-pre-line">{s.body}</p>
                {s.bullets && (
                  <ul className="mt-5 grid sm:grid-cols-2 gap-2 text-sm">
                    {s.bullets.map((b) => (
                      <li key={b} className="flex items-start gap-2"><CheckCircle2 size={16} className="text-blue-400 mt-0.5" />{b}</li>
                    ))}
                  </ul>
                )}
              </div>
          ...
          <article className="rounded-3xl glass-strong p-8">
            ...
            <div className="mt-5 grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {p.applications.map((a) => (
                <div key={a} className="rounded-xl glass p-4 text-sm">{a}</div>
              ))}
            </div>
          </article>
          ...
          <article>
            ...
            <div className="mt-5 grid sm:grid-cols-3 gap-3">
              {p.related.map((r) => (
                <Link key={r.to} to={r.to} className="rounded-xl glass p-4 text-sm flex items-center justify-between hover:bg-blue-500/5">
